...
.PARAMETER TenantName
    SharePoint tenant name (e.g., "safousa" for safousa.sharepoint.com)

.PARAMETER ClientName
    Client or organization name used as the report folder name.
    (e.g., "SAFO-LLC" creates folder: SAFO-LLC_SPO_Audit_20260217_150000)

.PARAMETER AdminUPN
    UPN of the admin account (e.g., cmsp365admin@safousa.com).
...
.EXAMPLE
    # Step 1 - pre-connect:
    Connect-SPOService -Url "https://safousa-admin.sharepoint.com"

    # Step 2 - run:
    .\SPO-TenantSecurityAudit.ps1 -TenantName "safousa" -ClientName "SAFO-LLC" -AdminUPN "cmsp365admin@safousa.com"
...
function Get-DeepPermissions {
    ...
    if (-not $script:PnPCreds) {
        Write-AuditLog "Enter admin credentials for PnP scan (prompted once, reused for all sites):" "INFO"
        $script:PnPCreds = Get-Credential -Message "SharePoint admin credentials for PnP scan"
    }
    ...
    foreach ($site in $filteredSites) {
        $counter++
        if ($counter % 10 -eq 0) {
            Write-AuditLog "  PnP Progress: $counter / $($filteredSites.Count)" "INFO"
        }
        try {
            Connect-PnPOnline -Url $site.Url -Credentials $script:PnPCreds -ErrorAction Stop -WarningAction SilentlyContinue

            $uniqueLists = Get-PnPList -ErrorAction SilentlyContinue |
            ...
            foreach ($list in $uniqueLists) {
                [void]$permData.Add([PSCustomObject]@{
                    ...
                    Note          = "Unique permissions - review access"
                })
            }
...
function Get-AuditLogEvents {
    ...
    if (-not (Get-Command Search-UnifiedAuditLog -ErrorAction SilentlyContinue)) {
        ...
        try {
            ...
            Write-AuditLog "Could not connect for audit log. Skipping Section 6: $_" "ERROR"
            return
        }
    }
...
<div class="footer">
  ...
  <div class="sig">
    Generated by <strong>Luis Z Guzman Garcia (KillBillKill98)</strong> |
